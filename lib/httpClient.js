...
class HttpClient {
  ...
  request(httpMethod, username, requestUrl, firstParam={}, secondParam={}) {
    let headers = {
      ...
      'user-agent': 'fantasyclient-nodejs/0.1.0'
    };
    ...
    return new Promise(function(resolve, reject) {
      ...
      let response = thisInstance.http_client({
        ...
        if(res.data.hasOwnProperty('status_code') && res.data.status_code >= 400 && res.data.status_code != 407) {
          ...
          reject(
            new FantasyError(
              `Fntasy RMT API is returning API error. HTTP status code: ${res.data.status_code}. API response: ${JSON.stringify(res.data)}`,
        ...
        if(typeof res !== 'undefined' && res.status >= 400) {
          reject(
            new FantasyError(
              `Fntasy RMT API is returning API error. HTTP status code: ${res.status}. API response: ${JSON.stringify(res.data)}`,
              res.status,
              res.data,
              res
            )
        ...
        // Reject API undefined HTTP response 
          ...
          reject(
            new FantasyError(
              `Fntasy RMT API request failed. HTTP response not found, likely connection failure, with message: ${JSON.stringify(err.message)}`,
              null,
              null,
              err
            )
