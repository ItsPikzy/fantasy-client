...
const FantasyError = require('./fantasyError');

/**
 * Wrapper of Axios to do API request to Fantasy RMT API
...
class HttpClient {
  ...
  request(httpMethod, username, requestUrl, firstParam = {}, secondParam = {}) {
    let headers = {
      ...
      'user-agent': 'fantasyclient-nodejs/1.0.0'
    };
    ...
    return new Promise(function(resolve, reject) {
      ...
      if(typeof reqBodyPayload === 'string' || reqBodyPayload instanceof String) {
        try {
          ...
          reject(new FantasyError(`fail to parse 'body parameters' string as JSON. Use JSON string or Object as 'body parameters'. with message: ${err}`));
        }
      }
      // Reject if param is not JSON
      if(typeof reqQueryParam === 'string' || reqQueryParam instanceof String) {
        try {
          ...
          reject(new FantasyError(`fail to parse 'query parameters' string as JSON. Use JSON string or Object as 'query parameters'. with message: ${err}`));
        }
      }
      ...
      thisInstance.http_client({
        ...
        if(res.data.hasOwnProperty('status_code') && res.data.status_code >= 400 && res.data.status_code != 407) {
          ...
          reject(
            new FantasyError(
              `Fantasy RMT API is returning API error. HTTP status code: ${res.data.status_code}. API response: ${JSON.stringify(res.data)}`,
        ...
        if(typeof res !== 'undefined' && res.status >= 400) {
          reject(
            new FantasyError(
              `Fantasy RMT API is returning API error. HTTP status code: ${res.status}. API response: ${JSON.stringify(res.data)}`,
              res.status,
              res.data,
              res
            )
        ...
        // Reject API undefined HTTP response 
          ...
          reject(
            new FantasyError(
              `Fantasy RMT API request failed. HTTP response not found, likely connection failure, with message: ${JSON.stringify(err.message)}`,
              null,
              null,
              err
            )
