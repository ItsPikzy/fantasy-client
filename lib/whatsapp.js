...
class WhatsApp {
  ...
  /**
   * Do `/phoneIsActive` API request to Fantasy RMT WhatsApp API
   * @param  {Object} parameter - object of WhatsApp API JSON body as parameter, will be converted to JSON (more params detail refer to: https://docs.rmtid.xyz/whatsapp)
   * @return {Promise} - Promise contains Object from JSON decoded response
   */
  phoneIsActive(phoneNumber) {
    let apiUrl = this.apiConfig.getWhatsAppApiBaseUrl()+'/v1/phoneIsActive';
    let responsePromise = this.httpClient.request(
      'post',
      this.apiConfig.get().whatsAppKey,
      apiUrl,
  ...
  /**
   * Do `/statusMessage` API request to Fantasy RMT WhatsApp API
   * @queueId  {String} parameter - object of WhatsApp API JSON body as parameter, will be converted to JSON (more params detail refer to: https://docs.rmtid.xyz/whatsapp)
   * @return {Promise} - Promise contains Object from JSON decoded response
   */
  statusMessage(queueId) {
    let apiUrl = this.apiConfig.getWhatsAppApiBaseUrl()+'/v1/statusMessage';
    let responsePromise = this.httpClient.request(
      'post',
      this.apiConfig.get().whatsAppKey,
      apiUrl,
